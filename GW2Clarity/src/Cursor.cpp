...
Cursor::Cursor(ComPtr<ID3D11Device>& dev)
	: activateCursor_("activate_cursor", "Toggle Cursor", "General") {
	Load();
	...
	cursorPS_ = {
		makePS("Circle"), makePS("Square"), makePS("Cross")
	};
...
void Cursor::DrawMenu(Keybind** currentEditedKeybind) {
	if(ImGui::BeginListBox("##LayersList", ImVec2(-FLT_MIN, 0.f))) {
		...
		{
			if (ImGui::Selectable(std::format("{}##Layer", l.name).c_str(), selectedLayerId_ == lid || currentHoveredLayer_ == lid, ImGuiSelectableFlags_AllowItemOverlap))
			{
	...
	if (selectedLayerId_ != UnselectedSubId)
	{
		...
		case CursorType::CROSS:
			saveCheck(ImGui::DragFloat("Cross Thickness", &editLayer.secondaryThickness, 0.05f, 1.f, std::min(editLayer.dims.x, editLayer.dims.y)));
