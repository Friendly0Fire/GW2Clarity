...
extern "C"
{
    __declspec(dllimport) void Direct3D11Loader_PrePresentSwapChain();
...
const wchar_t* g_windowTitle = L"GW2Mock";
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow) {
    ...
    g_hWnd = CreateWindow(g_windowClass,
                          g_windowTitle,
...
HRESULT InitDevice() {
    ...
    Direct3D11Loader_PostCreateSwapChain(g_hWnd, g_pd3dDevice, g_pSwapChain);
...
void Render() {
    ...
    const float color[] = { 0.1f, 0.f, 0.f, 0.f };
    g_pImmediateContext->ClearRenderTargetView(g_pRenderTargetView, color);
    Direct3D11Loader_PrePresentSwapChain();
